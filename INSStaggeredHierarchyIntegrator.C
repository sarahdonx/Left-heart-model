...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////Added By Liuyang Feng on June, 2023, some variables needed for later calculations
	 d_KE_cc_var = new CellVariable<NDIM, double>(d_object_name + "::KE_cc", NDIM);
	 d_CW_cc_var = new CellVariable<NDIM, double>(d_object_name + "::CW_cc", NDIM);
	 d_PW_cc_var = new CellVariable<NDIM, double>(d_object_name + "::PW_cc", NDIM);
	 d_VW_cc_var = new CellVariable<NDIM, double>(d_object_name + "::VW_cc", NDIM);
	 d_FSI_cc_var = new CellVariable<NDIM, double>(d_object_name + "::FSI_cc", NDIM);
	 d_heart_cc_var = new CellVariable<NDIM, double>(d_object_name + "::LA_cc", NDIM);
	 ///
	 d_KE_sc_var = new SideVariable<NDIM, double>(d_object_name + "::KE_sc");
	 d_CW_sc_var = new SideVariable<NDIM, double>(d_object_name + "::CW_sc");
	 d_PW_sc_var = new SideVariable<NDIM, double>(d_object_name + "::PW_sc");
	 d_VW_sc_var = new SideVariable<NDIM, double>(d_object_name + "::VW_sc");
	 d_FSI_sc_var = new SideVariable<NDIM, double>(d_object_name + "::FSI_sc");
	 d_KE_U_sc_var = new SideVariable<NDIM, double>(d_object_name + "::KE_ssc");
	 d_PP_sc_var = new SideVariable<NDIM, double>(d_object_name + "::CW_ssc");
	 d_VW_US_sc_var = new SideVariable<NDIM, double>(d_object_name + "::VW_ssc");
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////Added By Liuyang Feng on June, 2023
                                                               ...
                                                               Pointer<GriddingAlgorithm<NDIM> > gridding_alg)
{
    ...
    registerVariable(d_N_old_current_idx,
                     ...
                     d_N_coarsen_type,
                     d_N_refine_type);
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////Added By Liuyang Feng on Jun, 2023, some variables needed later
 //register needed context: cell var->cell ghosts, side var->side ghosts, plot var-> no ghost 
d_in_KE_U_scratch_idx=-1;
d_in_PP_scratch_idx=-1;
d_in_VW_US_scratch_idx=-1;
d_in_KE_scratch_idx=-1;
d_in_CW_scratch_idx=-1;
d_in_PW_scratch_idx=-1;
d_in_VW_scratch_idx=-1;
d_in_FSI_scratch_idx=-1;
d_in_KE_idx=-1;
d_in_PW_idx=-1;
d_in_CW_idx=-1;
d_in_VW_idx=-1;
d_in_FSI_idx=-1;
d_in_heart_idx=-1;
registerVariable(d_in_KE_U_scratch_idx, d_KE_U_sc_var, side_ghosts, getScratchContext());
registerVariable(d_in_PP_scratch_idx, d_PP_sc_var, side_ghosts, getScratchContext());
registerVariable(d_in_VW_US_scratch_idx, d_VW_US_sc_var, side_ghosts, getScratchContext());
registerVariable(d_in_KE_scratch_idx, d_KE_sc_var, side_ghosts, getScratchContext());
registerVariable(d_in_CW_scratch_idx, d_CW_sc_var, side_ghosts, getScratchContext());
registerVariable(d_in_PW_scratch_idx, d_PW_sc_var, side_ghosts, getScratchContext());
registerVariable(d_in_VW_scratch_idx, d_VW_sc_var, side_ghosts, getScratchContext());
registerVariable(d_in_FSI_scratch_idx, d_FSI_sc_var, side_ghosts, getScratchContext());					 
registerVariable(d_in_KE_idx, d_KE_cc_var, no_ghosts, getCurrentContext());
registerVariable(d_in_CW_idx, d_CW_cc_var, no_ghosts, getCurrentContext());
registerVariable(d_in_PW_idx, d_PW_cc_var, no_ghosts, getCurrentContext());
registerVariable(d_in_VW_idx, d_VW_cc_var, no_ghosts, getCurrentContext());
registerVariable(d_in_FSI_idx, d_FSI_cc_var, no_ghosts, getCurrentContext());
registerVariable(d_in_heart_idx, d_heart_cc_var, no_ghosts, getCurrentContext());
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////Added By Liuyang Feng on June, 2023
    ...
    if (d_visit_writer)
    {
        ...
        if (d_output_P)
        {
			...
			////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////Added By Liuyang Feng on June, 2023, these are the quantities to ouput to visIt 
			d_visit_writer->registerPlotQuantity("KE", "VECTOR", d_in_KE_idx, 0, d_U_scale);// kinetic energy rate 
            for (unsigned int d = 0; d < NDIM; ++d)
            {
                if (d == 0) d_visit_writer->registerPlotQuantity("KE_x", "SCALAR", d_in_KE_idx, d, d_U_scale);
                if (d == 1) d_visit_writer->registerPlotQuantity("KE_y", "SCALAR", d_in_KE_idx, d, d_U_scale);
                if (d == 2) d_visit_writer->registerPlotQuantity("KE_z", "SCALAR", d_in_KE_idx, d, d_U_scale);
            }
			/////////////////////////
			d_visit_writer->registerPlotQuantity("CW", "VECTOR", d_in_CW_idx, 0, d_U_scale);//energy flux rate 
            for (unsigned int d = 0; d < NDIM; ++d)
            {
                if (d == 0) d_visit_writer->registerPlotQuantity("CW_x", "SCALAR", d_in_CW_idx, d, d_U_scale);
                if (d == 1) d_visit_writer->registerPlotQuantity("CW_y", "SCALAR", d_in_CW_idx, d, d_U_scale);
                if (d == 2) d_visit_writer->registerPlotQuantity("CW_z", "SCALAR", d_in_CW_idx, d, d_U_scale);
            }
			/////////////////////////
			d_visit_writer->registerPlotQuantity("PW", "VECTOR", d_in_PW_idx, 0, d_U_scale);//Pressure work  
            for (unsigned int d = 0; d < NDIM; ++d)
            {
                if (d == 0) d_visit_writer->registerPlotQuantity("PW_x", "SCALAR", d_in_PW_idx, d, d_U_scale);
                if (d == 1) d_visit_writer->registerPlotQuantity("PW_y", "SCALAR", d_in_PW_idx, d, d_U_scale);
                if (d == 2) d_visit_writer->registerPlotQuantity("PW_z", "SCALAR", d_in_PW_idx, d, d_U_scale);
            }
			/////////////////////////
			d_visit_writer->registerPlotQuantity("VW", "VECTOR", d_in_VW_idx, 0, d_U_scale);// dissipation energy rate 
            for (unsigned int d = 0; d < NDIM; ++d)
            {
                if (d == 0) d_visit_writer->registerPlotQuantity("VW_x", "SCALAR", d_in_VW_idx, d, d_U_scale);
                if (d == 1) d_visit_writer->registerPlotQuantity("VW_y", "SCALAR", d_in_VW_idx, d, d_U_scale);
                if (d == 2) d_visit_writer->registerPlotQuantity("VW_z", "SCALAR", d_in_VW_idx, d, d_U_scale);
            }
			/////////////////////////
			d_visit_writer->registerPlotQuantity("FSI", "VECTOR", d_in_FSI_idx, 0, d_U_scale);//elastic energy changing rate 
            for (unsigned int d = 0; d < NDIM; ++d)
            {
                if (d == 0) d_visit_writer->registerPlotQuantity("FSI_x", "SCALAR", d_in_FSI_idx, d, d_U_scale);
                if (d == 1) d_visit_writer->registerPlotQuantity("FSI_y", "SCALAR", d_in_FSI_idx, d, d_U_scale);
                if (d == 2) d_visit_writer->registerPlotQuantity("FSI_z", "SCALAR", d_in_FSI_idx, d, d_U_scale);
            }
			/////////////////////////
			d_visit_writer->registerPlotQuantity("HEART", "VECTOR", d_in_heart_idx, 0, d_U_scale);// this one is just to flag the chamber space
            for (unsigned int d = 0; d < NDIM; ++d)
            {
                if (d == 0) d_visit_writer->registerPlotQuantity("Heart_x", "SCALAR", d_in_heart_idx, d, d_U_scale);
                if (d == 1) d_visit_writer->registerPlotQuantity("Heart_y", "SCALAR", d_in_heart_idx, d, d_U_scale);
                if (d == 2) d_visit_writer->registerPlotQuantity("Heart_z", "SCALAR", d_in_heart_idx, d, d_U_scale);
            }
			////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////Added By Liuyang Feng on June, 2023
        }
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////Added By Liuyang Feng on June, 2023, start the integration for each energy term
	int Energy_time_step = getIntegratorStep() + 1;
	if (current_time >= 5.0*(new_time-current_time) && Energy_time_step%500 == 0 && d_Energy_flag==1)//Every 500 timestep update once, no need to do it every time step
	{
		//Here is to calculate energy term for each term.
		//For KE term
		 d_hier_sc_data_ops->linearSum(d_in_KE_U_scratch_idx,1.0/(new_time-current_time),d_U_new_idx,-1.0/(new_time-current_time),d_U_current_idx);
		//for CW term
		//for PW term
		d_hier_cc_data_ops->copyData(d_P_scratch_idx, d_P_new_idx);
		//for VW term
		d_hier_sc_data_ops->linearSum(d_in_VW_US_scratch_idx,0.5,d_U_new_idx,0.5,d_U_current_idx);
		d_hier_sc_data_ops->copyData(d_U_scratch_idx, d_in_VW_US_scratch_idx);
		// Setup the patch boundary filling objects
		d_LYF_U_fill_pattern = new SideNoCornersFillPattern(1, false, false, true);
		d_LYF_P_fill_pattern = new CellNoCornersFillPattern(1, false, false, true);
		d_LYF_U_bc_coefs = getVelocityBoundaryConditions();
		d_LYF_P_bc_coef = getPressureBoundaryConditions();
		typedef HierarchyGhostCellInterpolation::InterpolationTransactionComponent InterpolationTransactionComponent;
		std::vector<InterpolationTransactionComponent> in_transaction_comps(2);
		 in_transaction_comps[0]= InterpolationTransactionComponent(d_U_scratch_idx,
															   d_in_VW_US_scratch_idx, 
														 "NONE",
														 true,
														 "CUBIC_COARSEN",
														 "LINEAR",
														 false,
														 d_LYF_U_bc_coefs,d_LYF_U_fill_pattern);
		in_transaction_comps[1] = InterpolationTransactionComponent(d_P_scratch_idx,
																 "NONE",
																  true,
															   "CUBIC_COARSEN",
															  "LINEAR",
																   false,
																 d_LYF_P_bc_coef,
																 d_LYF_P_fill_pattern);												 
		SAMRAI::tbox::Pointer<IBTK::HierarchyGhostCellInterpolation> d_LYF_hier_bdry_fill = new HierarchyGhostCellInterpolation();
		d_LYF_hier_bdry_fill->initializeOperatorState(in_transaction_comps, d_hierarchy);
		d_LYF_hier_bdry_fill->resetTransactionComponents(in_transaction_comps);
		d_LYF_hier_bdry_fill->setHomogeneousBc(false);
		StaggeredStokesPhysicalBoundaryHelper::setupBcCoefObjects(d_LYF_U_bc_coefs,
																  d_LYF_P_bc_coef,
																  d_U_scratch_idx,
															   d_P_scratch_idx,false);	
		d_LYF_hier_bdry_fill->fillData(new_time);
		StaggeredStokesPhysicalBoundaryHelper::resetBcCoefObjects(d_LYF_U_bc_coefs, d_LYF_P_bc_coef);
		d_LYF_hier_bdry_fill->resetTransactionComponents(in_transaction_comps);										  
			d_hier_math_ops->grad(d_in_PP_scratch_idx, d_PP_sc_var,
				 false,
				  1.0,
				  d_P_scratch_idx,
				  d_P_var,
			...
			PoissonSpecifications U_LYF_problem_coefs( "U_LYF_problem_coefs");
			U_LYF_problem_coefs.setCConstant(0.0);
			U_LYF_problem_coefs.setDConstant(0.04);
														  
			d_hier_math_ops->laplace(
			d_in_VW_US_scratch_idx, d_VW_US_sc_var, U_LYF_problem_coefs, d_U_scratch_idx, d_U_var, NULL, new_time);	
		
			d_LYF_hier_bdry_fill->deallocateOperatorState();
			d_LYF_hier_bdry_fill.setNull();
		  //  in_transaction_comps.clear();
			d_LYF_U_fill_pattern.setNull();
			d_LYF_P_fill_pattern.setNull();
		//////////////////////Now calculate each term: dot product 
		const int N_idx = d_N_vec->getComponentDescriptorIndex(0);
		////////////////multiply terms
		//KE
		d_hier_sc_data_ops->multiply(d_in_KE_scratch_idx,d_in_KE_U_scratch_idx,d_U_scratch_idx,false);
		d_hier_sc_data_ops->multiply(d_in_PW_scratch_idx,d_in_PP_scratch_idx,d_U_scratch_idx,false);
		d_hier_sc_data_ops->multiply(d_in_VW_scratch_idx,d_in_VW_US_scratch_idx,d_U_scratch_idx,false);
		d_hier_sc_data_ops->multiply(d_in_CW_scratch_idx,N_idx,d_U_scratch_idx,false);
		d_hier_sc_data_ops->multiply(d_in_FSI_scratch_idx,d_F_new_idx,d_U_scratch_idx,false);
		//
		d_hier_sc_data_ops->copyData(d_U_scratch_idx, d_in_KE_scratch_idx);
		d_hier_math_ops->interp(d_in_KE_idx, d_KE_cc_var, d_U_scratch_idx, d_U_var, d_no_fill_op, d_integrator_time, true);
		//
		d_hier_sc_data_ops->linearSum(d_U_scratch_idx, -1.0,d_in_PW_scratch_idx,0.0,d_in_PW_scratch_idx);
		d_hier_math_ops->interp(d_in_PW_idx, d_PW_cc_var, d_U_scratch_idx, d_U_var, d_no_fill_op, d_integrator_time, true);
		//
		d_hier_sc_data_ops->copyData(d_U_scratch_idx, d_in_CW_scratch_idx);
		d_hier_math_ops->interp(d_in_CW_idx, d_CW_cc_var, d_U_scratch_idx, d_U_var, d_no_fill_op, d_integrator_time, true);
		//
		d_hier_sc_data_ops->copyData(d_U_scratch_idx, d_in_VW_scratch_idx);
		d_hier_math_ops->interp(d_in_VW_idx, d_VW_cc_var, d_U_scratch_idx, d_U_var, d_no_fill_op, d_integrator_time, true);
		//
		d_hier_sc_data_ops->copyData(d_U_scratch_idx, d_in_FSI_scratch_idx);
		d_hier_math_ops->interp(d_in_FSI_idx, d_FSI_cc_var, d_U_scratch_idx, d_U_var, d_no_fill_op, d_integrator_time, true);
		//////////////////////////////////////////////////////////
		...
		for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
		{
			...
			{
				Pointer<Patch<NDIM> > patch = level->getPatch(p());
				Pointer<CellData<NDIM,double> > heart_data = patch->getPatchData(d_in_heart_idx);
				Pointer<CellData<NDIM,double> > vol_data = patch->getPatchData(wgt_cc_idx);
				const Box<NDIM>& patch_box = patch->getBox();
									 ...
									 //Now for 2D 
											if (NDIM==2)
										   {
											...
											else if(NDIM==3)
											{
												if (d_Energy_flag==1)
												{
														(*heart_data)(i,1)=0;//only use first index to flag chamber
														(*heart_data)(i,2)=0;
														//Important!! This specifies the region we do the calculation 											  
													if( X[0]>=-4.0 && X[0]<=12.0 && X[1] >= -26.0 && X[1] <= -12.0 && X[2] >= 134.5 && X[2] <= 148.5)
													{
													  int count=0;
													  double V_1[NDIM];
													  double V_2[NDIM];
													  ...
													  double v[NDIM];
													  double w[NDIM];
													  double Dn[NDIM];//Dn is the plane normal direction
													  double ri;
													  double si;
													  double ti;													  
													  
													  ////loop over all the LV surface
													  for (int Vd = 0; Vd < d_LVVex_Number/3; ++Vd)
													  {
														 V_1[0]= d_LVVex_Point[(d_LVVex_List[Vd*3]-1)*3];
														 V_1[1]= d_LVVex_Point[(d_LVVex_List[Vd*3]-1)*3+1];
														 V_1[2]= d_LVVex_Point[(d_LVVex_List[Vd*3]-1)*3+2];
														 V_2[0]= d_LVVex_Point[(d_LVVex_List[Vd*3+1]-1)*3];
														 V_2[1]= d_LVVex_Point[(d_LVVex_List[Vd*3+1]-1)*3+1];
														 V_2[2]= d_LVVex_Point[(d_LVVex_List[Vd*3+1]-1)*3+2];											 
														 V_3[0]= d_LVVex_Point[(d_LVVex_List[Vd*3+2]-1)*3];
														 V_3[1]= d_LVVex_Point[(d_LVVex_List[Vd*3+2]-1)*3+1];
														 V_3[2]= d_LVVex_Point[(d_LVVex_List[Vd*3+2]-1)*3+2];
														 u[0]=V_2[0]-V_1[0];u[1]=V_2[1]-V_1[1];u[2]=V_2[2]-V_1[2];
														 v[0]=V_3[0]-V_1[0];v[1]=V_3[1]-V_1[1];v[2]=V_3[2]-V_1[2];
														 ////
														 Dn[0]=u[1]*v[2]-u[2]*v[1];Dn[1]=u[2]*v[0]-u[0]*v[2];Dn[2]=u[0]*v[1]-u[1]*v[0];
														 ri=((V_1[0]-X[0])*Dn[0]+(V_1[1]-X[1])*Dn[1]+(V_1[2]-X[2])*Dn[2])/(-1*Dn[1]);
														 if (ri>=0)
														 {
															 w[0]=X[0]-V_1[0];w[1]=X[1]-ri-V_1[1];w[2]=X[2]-V_1[2];
															 si=((u[0]*v[0]+u[1]*v[1]+u[2]*v[2])*(w[0]*v[0]+w[1]*v[1]+w[2]*v[2])-(v[0]*v[0]+v[1]*v[1]+v[2]*v[2])*(w[0]*u[0]+w[1]*u[1]+w[2]*u[2]))/((u[0]*v[0]+u[1]*v[1]+u[2]*v[2])*(u[0]*v[0]+u[1]*v[1]+u[2]*v[2])-(u[0]*u[0]+u[1]*u[1]+u[2]*u[2])*(v[0]*v[0]+v[1]*v[1]+v[2]*v[2]));
															 ti=((u[0]*v[0]+u[1]*v[1]+u[2]*v[2])*(w[0]*u[0]+w[1]*u[1]+w[2]*u[2])-(u[0]*u[0]+u[1]*u[1]+u[2]*u[2])*(w[0]*v[0]+w[1]*v[1]+w[2]*v[2]))/((u[0]*v[0]+u[1]*v[1]+u[2]*v[2])*(u[0]*v[0]+u[1]*v[1]+u[2]*v[2])-(u[0]*u[0]+u[1]*u[1]+u[2]*u[2])*(v[0]*v[0]+v[1]*v[1]+v[2]*v[2]));
													 ...
													 if (count%2==1) // inside LV
													 {
															(*heart_data)(i,0)=1;												
													 }
													 else//Not inside LV, search LA
													 {																						
															count=0;
															//loop here
															////loop over all the LA surface
															for (int Vd = 0; Vd < d_LAVex_Number/3; ++Vd)
															  {
																 V_1[0]= d_LAVex_Point[(d_LAVex_List[Vd*3]-1)*3];
																 V_1[1]= d_LAVex_Point[(d_LAVex_List[Vd*3]-1)*3+1];
																 V_1[2]= d_LAVex_Point[(d_LAVex_List[Vd*3]-1)*3+2];
																 V_2[0]= d_LAVex_Point[(d_LAVex_List[Vd*3+1]-1)*3];
																 V_2[1]= d_LAVex_Point[(d_LAVex_List[Vd*3+1]-1)*3+1];
																 V_2[2]= d_LAVex_Point[(d_LAVex_List[Vd*3+1]-1)*3+2];											 
																 V_3[0]= d_LAVex_Point[(d_LAVex_List[Vd*3+2]-1)*3];
																 V_3[1]= d_LAVex_Point[(d_LAVex_List[Vd*3+2]-1)*3+1];
																 V_3[2]= d_LAVex_Point[(d_LAVex_List[Vd*3+2]-1)*3+2];
																 u[0]=V_2[0]-V_1[0];u[1]=V_2[1]-V_1[1];u[2]=V_2[2]-V_1[2];
																 v[0]=V_3[0]-V_1[0];v[1]=V_3[1]-V_1[1];v[2]=V_3[2]-V_1[2];
																 ////
																 Dn[0]=u[1]*v[2]-u[2]*v[1];Dn[1]=u[2]*v[0]-u[0]*v[2];Dn[2]=u[0]*v[1]-u[1]*v[0];
															...
															if (count%2==1)// inside LA 
															 {
																	(*heart_data)(i,0)=3;												
															 }
															 else//Not in LA either
															 {
																 (*heart_data)(i,0)=0;																 
															 }
														}																													
